...
import { forwardRef, ForwardRefRenderFunction, useCallback, useEffect, useImperativeHandle, useState, MouseEvent, ChangeEvent } from "react";
...
type SelectTrackers = 'my' | 'my_products' | 'issued' | 'unissued' | 'requested'
...
type IssuedTrackersProps = {
  ...
  handleTrackerSelect?:(tracker:Tracker) => void,
}
...
const IssuedTrackers: ForwardRefRenderFunction<IssuedTrackersHandle, IssuedTrackersProps> = ({ provider, signedInAddress, roles, displayAddress, _showTrackers='issued',handleTrackerSelect }, ref) => {
  ...
  if(!displayAddress){trackerSelectors.push({key: 'my', value: 'issued to me'})} 
  ...
  const fetchTrackers = useCallback(async (_page: number, _pageSize: number, _query: string[]) => {
    ...
    try {
      ...
      */
      ...
      for (let i = 1; i <= numOfUniqueTrackers; i++) {
        ...
        if(typeof tracker === "object" && (!displayAddress || tracker.trackee.toLowerCase()===displayAddress.toLowerCase())){
    ...
    setQuery(_query);
  }, [provider, signedInAddress, displayAddress]);
  ...
  function renderTrackersTable(trackers: Tracker[], fetching:boolean, provider?: Web3Provider | JsonRpcProvider){
    ...
    return(<Table hover size="sm" key={showTrackers+"Table"}>
      <thead>
        <tr>
          <th>ID</th>
          <th>{showTrackers==='my_products' ? "My Emissions":"Total Emissions"}</th>
          <th>Products</th>
          <th>Description</th>
        </tr>
      </thead>
        ...
        {(trackers !== [] && !fetching) &&
          trackers.map((tracker:Tracker) => (
            <tr key={tracker.trackerId+showTrackers} style={{ backgroundColor: rowShading(tracker?.products![1]?.emissionsFactor!)}} onClick={() => handleOpenTrackerInfoModal(tracker)} onMouseOver={pointerHover}>
              ...
              <td>{showTrackers === 'my_products' ? tracker?.myProductsTotalEmissions!.toLocaleString('en-US') : tracker.totalEmissions.toLocaleString('en-US')}</td>
              ...
              <td>
                ...
                {(isDealer && tracker.auditor === "0x0000000000000000000000000000000000000000") &&
                  (<><Button className="mb-3" variant="outline-dark" href={"/track/"+tracker.trackerId}> Add token</Button>
                    {provider && <Button className="mb-3" variant="outline-dark" onClick={async() => await verifyTracker(provider, tracker.trackerId)}>Verify</Button>}</>)
                }
              </td>
            </tr>)
        )}
  ...
  return (
    <>
      ...
      {isDealer && <Button style={{display: 'inline'}} className="mb-3" variant="outline-dark" href="/track/0"> Issue </Button>}
      <div className="mt-4">
        {showTrackers==='my' && <>
